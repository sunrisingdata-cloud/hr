...
// 설정 저장
// 기본급 데이터 업로드 및 저장
function uploadBasicSalaryData(jsonDataString) {
  try {
    const basicSalaryData = JSON.parse(jsonDataString);
    const properties = PropertiesService.getUserProperties();
    properties.setProperty('basicSalaryData', jsonDataString);
    
    // 메타정보도 저장
    const meta = {
      uploadedDate: new Date().toLocaleDateString('ko-KR'),
      hobonCount: Object.keys(basicSalaryData).length
    };
    properties.setProperty('basicSalaryMeta', JSON.stringify(meta));
    
    return { success: true, message: '기본급 데이터 저장 완료' };
  } catch(error) {
...
// 저장된 기본급 데이터 불러오기
function getBasicSalaryData() {
  const properties = PropertiesService.getUserProperties();
  const data = properties.getProperty('basicSalaryData');
  return data ? JSON.parse(data) : null;
}

// 특정 직원의 기본급 계산
function getSalaryFromTable(grade, hobon) {
  var basicSalaryData = getBasicSalaryData();
  
  if (basicSalaryData == null) {
    return 0;
  }
  
  if (basicSalaryData[hobon] == null) {
    return 0;
  }
  
  if (basicSalaryData[hobon][grade] == null) {
    return 0;
  }
  
  return basicSalaryData[hobon][grade];
}

function calculateAnnualSalary(empId) {
  
  const hobongSheet = SpreadsheetApp.openById(SS_ID).getSheetByName('호봉관리');
  const data = hobongSheet.getDataRange().getValues();
  
  
  const empRow = data.find(row => row[0] === empId);
  
  if (!empRow) {
    return null;
  }
  
  const empName = empRow[1];
  const grade = empRow[2];
  const hobon = empRow[3];
  
  
  const basicSalaryData = getBasicSalaryData();
  
  if (basicSalaryData) {
    if (basicSalaryData[hobon]) {
    } else {
    }
  }
  
  const baseSalary = getSalaryFromTable(grade, hobon);
  ...
  for (let month = 1; month <= 12; month++) {
    monthlySalary[month] = {
      기본급: baseSalary,
      정액급식비: settings.allowances.mealSubsidy,
      명절휴가비: (month === 2 || month === 9) ? Math.floor(baseSalary * 0.6) : 0,
      가족수당: settings.allowances.spouse || 0,
      연장근로수당: 0,
      직책수당: 0,
      학위인정수당: 0,
      체력단련비: 0,
      하계휴가비: (month === 7 || month === 8) ? 50000 : 0
    };
  }
...
// =========================================================================
// 설정 저장/로드 (PropertiesService)
// =========================================================================

function saveSettings(settingsJson, year) {
  const properties = PropertiesService.getUserProperties();
  const key = year ? 'settingsData_' + year : 'settingsData';
  properties.setProperty(key, settingsJson);
  return { success: true, message: '설정이 저장되었습니다.' };
}

function loadSettings(year) {
  const properties = PropertiesService.getUserProperties();
  const key = year ? 'settingsData_' + year : 'settingsData';
  let settingsJson = properties.getProperty(key);
  // 연도별 설정이 없으면 레거시 단일 설정으로 폴백
  if (!settingsJson && year) {
    settingsJson = properties.getProperty('settingsData');
  }
...
function getAllowancesByYear(year) {
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      const yearVal = data[i][0];
      const name = data[i][1];
      const frequency = data[i][2];
      const amount = data[i][3];
      
      if (yearVal == year && name && name !== '가족수당') {
        allowances.push({
          name: name,
          frequency: frequency,
          amount: amount
...
function getFamilyAllowances(year) {
  try {
    const sheet = SpreadsheetApp.openById(SS_ID).getSheetByName('제수당');
    ...
    for (let i = 1; i < data.length; i++) {
      const yearVal = data[i][0];
      const name = data[i][1];
      const jsonStr = data[i][4]; // E열: JSON 형식 비고
      
      if (yearVal == year && name === '가족수당' && jsonStr) {
        try {
...
// =========================================================================
// 세금/퇴직금 시트에 2026년 요율 자동 입력 (B열 항목 매칭 → C열 비율)
// Apps Script 편집기에서 fillTaxRates 실행 (연도 기본값 현재연도)
// =========================================================================
function fillTaxRates(year) {
  year = year || new Date().getFullYear();
  const ss = SpreadsheetApp.openById(SS_ID);

  // B2에 '국민연금'이 있는 시트를 세금/퇴직금 시트로 인식
  let sheet = null;
  const sheets = ss.getSheets();
  for (let i = 0; i < sheets.length; i++) {
    const b2 = sheets[i].getRange('B2').getValue();
    if (b2 && b2.toString().indexOf('국민연금') !== -1) { sheet = sheets[i]; break; }
  }
  if (!sheet) return { success: false, message: '세금/퇴직금 시트를 찾을 수 없습니다 (B2에 국민연금(근) 필요).' };

  // 2026년 기준 요율(%). 장기요양은 건강보험료 대비 %, 주민세는 소득세 대비 %.
  const rates = {
    '국민연금(근)': 4.75,
    '국민연금(사)': 4.75,
    '건강보험(근)': 3.595,
    '건강보험(사)': 3.595,
    '장기요양(근)': 13.14,   // 건강보험료 × 13.14%
    '장기요양(사)': 13.14,
    '고용보험(근)': 0.9,
    '고용보험(사)': 1.15,    // 실업급여 0.9 + 고용안정·직능 0.25 (150인 미만)
    '산재보험(사)': 0.66,    // 사회복지서비스업 0.6 + 출퇴근재해 0.06 (실제율은 공단 확인)
    '퇴직적립금(사)': 8.33,  // 월 평균임금의 1/12
    '주민세': 10             // 소득세 × 10% (지방소득세)
    // '소득세'는 간이세액표 기반이라 단일 비율로 넣지 않음 (별도 처리)
  };
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (y == year && rates.hasOwnProperty(item)) {
      sheet.getRange(i + 1, 3).setValue(rates[item]); // C열
      updated++;
    }
  }
  return { success: true, message: updated + '개 요율 입력 완료 (소득세 제외)' };
}
...
// 클라이언트/서버 공통: 제수당 설정을 JSON 문자열로 (loadSettings 대체)
...
// =========================================================================
// [근태 CSV 업로드] code.gs 맨 아래에 추가 (이름 att_ 로 충돌 없음)
// 화면에서 CSV 파싱 후 rows 배열을 넘겨받아 근태기록 시트에 반영
// - 직원 매칭: 직원명부 Z열(근태기기ID) ↔ CSV의 ID
// - 중복: 같은 (직원ID + 날짜) 있으면 덮어쓰기
// 근태기록 시트: A직원ID B이름 C연월일 D출근시간 E퇴근시간
// =========================================================================

// 근태기기ID → {empId, name} 매핑 (직원명부 Z열 = index 25)
function att_deviceMap() {
  const sheet = SpreadsheetApp.openById(SS_ID).getSheetByName('직원명부');
  const map = {};
  if (!sheet) return map;
  const data = sheet.getDataRange().getValues();
  for (let i = 1; i < data.length; i++) {
    const empId = data[i][0] ? data[i][0].toString().trim() : '';
    const name = data[i][1];
    const devId = data[i][25] ? data[i][25].toString().trim() : ''; // Z열
    if (!empId || !devId) continue;
    map[devId] = { empId: empId, name: name };
  }
  return map;
}
...
// 화면에서 넘긴 CSV 행 반영
// rows: [{ devId, date, checkIn, checkOut }]
// 반환: { success, message, added, updated, skipped:[...] }
function importAttendanceRows(rows) {
  try {
    if (typeof rows === 'string') rows = JSON.parse(rows);
    if (!rows || !rows.length) return { success: false, message: '데이터가 없습니다.' };

    const ss = SpreadsheetApp.openById(SS_ID);
    let sheet = ss.getSheetByName('근태기록');
    if (!sheet) {
      sheet = ss.insertSheet('근태기록');
      sheet.getRange(1, 1, 1, 5).setValues([['직원ID', '이름', '연월일', '출근시간', '퇴근시간']]);
      sheet.getRange(1, 1, 1, 5).setFontWeight('bold').setBackground('#4472c4').setFontColor('#ffffff');
      sheet.setFrozenRows(1);
    }

    const map = att_deviceMap();

    // 기존 행 인덱스: 'empId|date' → 행번호
    const last = sheet.getLastRow();
    const existing = {};
    if (last >= 2) {
    ...
    for (let i = 0; i < rows.length; i++) {
      const r = rows[i];
      const devId = (r.devId || '').toString().trim();
      const date = att_date(r.date);
      const cin = att_time(r.checkIn);
      const cout = att_time(r.checkOut);
      if (!devId || !date) continue;

      const emp = map[devId];
      if (!emp) { skipped.push(devId + ' (' + date + ')'); continue; } // 기기ID 매칭 실패

      const key = emp.empId + '|' + date;
      if (existing[key]) {
        // 덮어쓰기
        sheet.getRange(existing[key], 1, 1, 5).setValues([[emp.empId, emp.name, date, cin, cout]]);
        updated++;
      } else {
        appends.push([emp.empId, emp.name, date, cin, cout]);
        existing[key] = -1; // 같은 배치 내 중복 방지
      }
    }
    ...
    const msg = '신규 ' + appends.length + '건, 갱신 ' + updated + '건';
